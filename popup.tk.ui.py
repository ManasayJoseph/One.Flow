...
from customtkinter import CTk, CTkToplevel, CTkFrame, CTkButton, CTkImage, CTkLabel
...
class App(CTk):
    ...
    def show_popup(self):
        ...
        img = CTkImage(dark_image=Image.open(image_path))
        img_resized = img.resize((150, 150))  # Resize the image

        # Display the image on a label with padding
        label = CTkLabel(popup, text="", image=img_resized, padx=20, pady=20)
        label.image = img_resized  # Keep a reference to avoid garbage collection
        label.pack()
